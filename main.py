#DECODED BY NETZ - MODIFIED WITH YANDEX EMAIL ONLY - OTP COMPULSORY FETCH
import os
import sys
import re
import random
        ...
        while True:
            ...
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    print(f"{Y}[!] Verification required for {email}, polling for OTP (2 min wait + retry)...{W}")
                    ...
                    if "checkpoint" in check_resp.text.lower() or "confirm" in check_resp.text.lower():
                        print(f"{Y}[!] Post-creation verification needed, fetching OTP (2 min wait + retry)...{W}")
