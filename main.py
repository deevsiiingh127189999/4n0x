...
def force_fetch_otp_from_email(email_address, timeout=120, mark_read=True):
    """
    Force fetch OTP from email - will wait until email arrives and extract OTP
    """
    try:
        ...
        last_resend_time = 0
        session_for_resend = None
        
        print(f"{Y}[*] Force fetching OTP from email: {email_address} (max {timeout}s){W}")
        
        while time.time() - start_time < timeout:
            # Search for emails to this address (TO and CC)
            status, messages = imap.search(None, f'OR TO "{email_address}" CC "{email_address}"')
                    ...
                    for response_part in msg_data:
                        if isinstance(response_part, tuple):
                            ...
                            if otp:
                                ...
                                imap.close()
                                imap.logout()
                                print(f"{G}[✓] OTP fetched successfully: {otp}{W}")
                                return otp
                            else:
                                print(f"{Y}[*] Email found but OTP pattern not detected yet. Retrying...{W}")
        ...
        imap.close()
        imap.logout()
        print(f"{R}[!] Timeout: No OTP found after {timeout} seconds{W}")
        return None
...
def confirm_account_with_auto_otp(session, email_address, max_retries=3):
    for attempt in range(max_retries):
        ...
        otp_code = force_fetch_otp_from_email(email_address, timeout=90, mark_read=True)
        if otp_code:
        ...
        if attempt == max_retries - 1:
            print(f"{Y}[!] Auto OTP failed after {max_retries} attempts. Please enter OTP manually (check email {email_address}):{W}")
            ...
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    if "checkpoint" in check_resp.text.lower():
                        ...
                        otp_code = force_fetch_otp_from_email(email, timeout=120)
            ...
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    if "checkpoint" in check_resp.text.lower() or "confirm" in check_resp.text.lower():
                        ...
                        # KEY FIX: Wait for OTP email with auto-resend
                        print(f"{Y}[*] Account created, now waiting for OTP email (max 120 sec)...{W}")
                        otp_code = force_fetch_otp_from_email(email, timeout=120)
        ...
        try:
            ...
            if "c_user" in login_coki:
                ...
                if "checkpoint" in check_resp.text.lower():
                    ...
                    # Wait for OTP email with auto-resend
                    print(f"{Y}[*] Waiting for OTP email for {email} (max 120 sec)...{W}")
                    otp_code = force_fetch_otp_from_email(email, timeout=120)
                    return {
                        "name": f"{firstname} {lastname}",
                        "email": email,
