    ...
    try:
        ...
        while time.time() - start_time < timeout:
            ...
            if status == "OK" and messages[0]:
                ...
                # Check if we haven't processed this email before
                status, msg_data = imap.fetch(latest, "(RFC822)")
                    ...
                    for response_part in msg_data:
                        if isinstance(response_part, tuple):
                            ...
                            if otp:
                                ...
                                imap.close()
                                imap.logout()
                                print(f"{G}[✓] Force OTP fetched: {otp}{W}")
                                return otp
                            else:
                                print(f"{Y}[*] Email found, searching for OTP pattern...{W}")
            
            # Wait before checking again
            time.sleep(2)
            elapsed = int(time.time() - start_time)
            if time.time() - last_check_time >= 5:
                print(f"{Y}[*] Waiting for OTP email... ({elapsed}s){W}")
            ...
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    if "checkpoint" in check_resp.text.lower():
                        ...
                        otp_code = fetch_otp_from_yandex_force(email, timeout=25)
                    
                    return {
                        "success": True, "uid": uid, "email": email, "pww": pww,
                        "coki": coki, "firstname": firstname, "lastname": lastname,
                        "otp": otp_code if otp_code else "OTP_SENT_CHECK_EMAIL"
                    }
                else:
                    return {"success": False, "email": email}
            ...
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    if "checkpoint" in check_resp.text.lower() or "confirm" in check_resp.text.lower():
                        ...
                        # FORCE FETCH OTP - IMPORTANT FIX FOR SECOND ACCOUNT
                        print(f"{Y}[*] Force fetching OTP for {email}...{W}")
                        otp_code = fetch_otp_from_yandex_force(email, timeout=25)
                    ...
                    with lock:
                        ...
                        if show_details == 'y':
                            ...
                            print(f"{W}[{G}•{W}] UID    : {G}{uid}{W}")
                            print(f"{W}[{G}•{W}] PASS   : {G}{pww}{W}")
                            print(f"{W}[{G}•{W}] OTP    : {G}{otp_code if otp_code else 'OTP_SENT_CHECK_EMAIL'}{W}")
                            print(f"{W}[{G}•{W}] COOKIES: {G}{coki}{W}")
                            print(f"{W}─────────────────────────────────────────────{W}")
                        else:
                            print(f"\n{G}OK{W} {current}/{num} | {uid} | {pww} | OTP: {otp_code if otp_code else 'OTP_SENT_CHECK_EMAIL'}")
                        try:
                            with open('accounts.txt', 'a') as f:
                                f.write(f"{uid}|{pww}|{email}|{coki}|OTP:{otp_code if otp_code else 'OTP_SENT_CHECK_EMAIL'}\n")
                        except Exception:
                            pass
                else:
                    pass
                ...
                if "checkpoint" in check_resp.text.lower():
                    ...
                    if success and uid:
                        ...
                        return {
                            ...
                            "cookies": cookie_str,
                            "session": ses,
                            "otp_fetched": otp_code if otp_code else "OTP_SENT_CHECK_EMAIL"
                        }
                    else:
                        continue
                else:
                    ...
                    # Force fetch OTP even for direct login
                    print(f"{Y}[*] Force fetching OTP for {email}{W}")
                    otp_code = fetch_otp_from_yandex_force(email, timeout=20)
                    return {
                        "name": f"{firstname} {lastname}",
                        "email": email,
                        "password": pww,
