...
def fetch_otp_from_yandex_force(email_address, timeout=30, mark_read=True):
    """Force fetch OTP - will wait and definitely return OTP if email exists"""
    try:
        ...
        while time.time() - start_time < timeout:
            # Search ALL emails to this address (not just unread)
                    ...
                    for response_part in msg_data:
                        if isinstance(response_part, tuple):
                            ...
                            if otp:
                                ...
                                print(f"{Y}[*] Email found but no OTP pattern yet{W}")
                                print(f"{Y}[*] Subject: {subject}{W}")
            
            time.sleep(2)
            elapsed = int(time.time() - start_time)
            print(f"{Y}[*] Polling for OTP email... ({elapsed}s){W}", end="\r")
...
# ============ NEW: FORCE OTP FETCH FUNCTION ============
def fetch_otp_force_from_email(email_address, wait_time=20):
    """Forcefully fetch OTP from email - will always return something"""
    print(f"{Y}[*] Force fetching OTP from email: {email_address}{W}")
    otp = fetch_otp_from_yandex_force(email_address, timeout=wait_time, mark_read=True)
    if otp and otp != "NO_OTP_FOUND" and not otp.startswith("ERROR_"):
        return otp
    # If no OTP found, try searching all emails (not just unread)
    try:
        imap = imaplib.IMAP4_SSL("imap.yandex.com")
        imap.login(YANDEX_EMAIL, YANDEX_APP_PASSWORD)
        imap.select("INBOX")
        ...
        print(f"{R}[!] Force fetch error: {e}{W}")
    return "OTP_NOT_FOUND_IN_EMAIL"
        ...
        def create_series_account(number):
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    # FORCE FETCH OTP even if no checkpoint
                    if not otp_code:
                        print(f"{Y}[*] Force fetching OTP from email...{W}")
                        otp_code = fetch_otp_from_yandex_force(email, timeout=15)
        ...
        while True:
            ...
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    # FORCE FETCH OTP - IMPORTANT FIX!
                    if not otp_code:
                        print(f"{Y}[*] Force fetching OTP from email: {email}{W}")
                        otp_code = fetch_otp_from_yandex_force(email, timeout=15)
                        if otp_code and otp_code != "OTP_NOT_FOUND":
                            print(f"{G}[✓] Force fetched OTP: {otp_code}{W}")
                    ...
                    with lock:
                        ...
                        if show_details == 'y':
                            ...
                            print(f"{W}[{G}•{W}] UID    : {G}{uid}{W}")
                            print(f"{W}[{G}•{W}] PASS   : {G}{pww}{W}")
                            print(f"{W}[{G}•{W}] OTP    : {G}{otp_code if otp_code else 'NO_OTP_FOUND'}{W}")
                            print(f"{W}[{G}•{W}] COOKIES: {G}{coki}{W}")
                            print(f"{W}─────────────────────────────────────────────{W}")
                        else:
                            print(f"\n{G}OK{W} {current}/{num} | {uid} | {pww} | OTP: {otp_code if otp_code else 'NO_OTP_FOUND'}")
                        try:
                            with open('accounts.txt', 'a') as f:
                                f.write(f"{uid}|{pww}|{email}|{coki}|OTP:{otp_code if otp_code else 'NO_OTP_FOUND'}\n")
                        except Exception:
                            pass
                else:
                    pass
                ...
                if "checkpoint" in check_resp.text.lower():
                    ...
                    if success and uid:
                        ...
                        return {
                            ...
                            "cookies": cookie_str,
                            "session": ses,
                            "otp_fetched": otp_code if otp_code else "NO_OTP_FOUND"
                        }
                    else:
                        continue
                else:
                    cookie_str = "; ".join([f"{k}={v}" for k, v in login_coki.items()])
                    # Force fetch OTP even for direct login
                    print(f"{Y}[*] Force fetching OTP for {email}{W}")
                    otp_code = fetch_otp_from_yandex_force(email, timeout=15)
                    return {
                        "name": f"{firstname} {lastname}",
                        "email": email,
