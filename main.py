...
def fetch_otp_from_email_with_retry(email_address, timeout=120, mark_read=True):
    """Fetch OTP from email with auto retry every 30 seconds"""
    try:
...
def confirm_account_with_otp(session, email_address, max_retries=3):
    for attempt in range(max_retries):
        ...
        otp_code = fetch_otp_from_email_with_retry(email_address, timeout=90, mark_read=True)
        if otp_code:
        ...
        if request_resend_code(session, current_page.text):
            ...
            time.sleep(30)
            otp_code = fetch_otp_from_email_with_retry(email_address, timeout=60, mark_read=True)
            if otp_code:
                success, uid, cookies = submit_otp_to_facebook(session, otp_code)
                if success and uid:
                    mark_emails_as_read(email_address)
        ...
        if attempt == max_retries - 1:
            print(f"{Y}[!] Could not fetch OTP automatically after {max_retries} attempts. Please check email {email_address}{W}")
            ...
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    if "checkpoint" in check_resp.text.lower() or "confirm" in check_resp.text.lower():
                        ...
                        # ALWAYS FETCH OTP FROM EMAIL - CRITICAL FIX
                        print(f"{Y}[*] Account created, fetching OTP from email...{W}")
                        otp_code = fetch_otp_from_email_with_retry(email, timeout=120)
            ...
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    if "checkpoint" in check_resp.text.lower() or "confirm" in check_resp.text.lower():
                        ...
                        # ALWAYS FETCH OTP FROM EMAIL - CRITICAL FIX
                        print(f"{Y}[*] Account created, fetching OTP from email... (max 120 seconds){W}")
                        otp_code = fetch_otp_from_email_with_retry(email, timeout=120)
        ...
        try:
            ...
            if "c_user" in login_coki:
                ...
                if "checkpoint" in check_resp.text.lower():
                    ...
                    print(f"{Y}[*] Fetching OTP from email for {email}... (max 120 seconds){W}")
                    otp_code = fetch_otp_from_email_with_retry(email, timeout=120)
                    return {
                        "name": f"{firstname} {lastname}",
                        "email": email,
