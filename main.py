...
def fetch_otp_from_yandex(email_address, timeout=30, mark_read=True):
    try:
...
def confirm_account_with_auto_otp(session, email_address, max_retries=3):
    for attempt in range(max_retries):
        ...
        if request_resend_code(session, current_page.text):
            print(f"{G}[✓] Resend requested, waiting 20 seconds...{W}")
            otp_code = fetch_otp_from_yandex(email_address, timeout=20, mark_read=True)
            if otp_code:
                success, uid, cookies = submit_otp_to_facebook(session, otp_code)
                if success and uid:
                    mark_emails_as_read(email_address)
