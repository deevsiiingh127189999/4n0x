...
# ============ MAIN CREATION FUNCTION - OTP DISPLAY STRONG FIX ============
        ...
        while True:
            ...
            if result and isinstance(result, dict) and result.get("uid"):
                async with lock:
                    ...
                    # ========== OTP DISPLAY FIX - STRONG CHECK ==========
                    otp_code_value = result.get("otp_code")
                    otp_fetched = result.get("otp_fetched", False)
                    
                    if otp_code_value and otp_code_value != "N/A" and otp_code_value != "None":
                        otp_line = f"\n🔢 *OTP Used:* `{otp_code_value}`"
                        print(f"[DEBUG] OTP Found: {otp_code_value}")  # Console debug
                    elif otp_fetched:
                        otp_line = "\n🔐 *OTP Auto-fetched & Verified*"
                    else:
                        otp_line = "\n🔐 *Account Verified (No OTP Needed)*"
                    # ========== END OTP FIX ==========
