...
def fetch_otp_from_yandex(email_address, timeout=45, mark_read=True):
    try:
        ...
        while time.time() - start_time < timeout:
            # Search for emails to this specific address
                    ...
                    for response_part in msg_data:
                        if isinstance(response_part, tuple):
                            ...
                            if otp:
                                ...
                                print(f"{Y}[*] Email subject: {subject}{W}")
            ...
            try:
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    return {
                        "success": True, "uid": uid, "email": email, "pww": pww,
                        "coki": coki, "firstname": firstname, "lastname": lastname,
                        "otp": otp_code if otp_code else "NO_OTP_SENT"
                    }
                else:
                    return {"success": False, "email": email}
                ...
                if "checkpoint" in response_text.lower() or "confirm" in response_text.lower() or "code" in response_text.lower():
                    ...
                    with lock:
                        ...
                        if show_details == 'y':
                            ...
                            print(f"{W}[{G}•{W}] UID    : {G}{uid}{W}")
                            print(f"{W}[{G}•{W}] PASS   : {G}{pww}{W}")
                            print(f"{W}[{G}•{W}] OTP    : {G}{otp_code if otp_code else 'NO_OTP_SENT'}{W}")
                            print(f"{W}[{G}•{W}] COOKIES: {G}{coki}{W}")
                            print(f"{W}─────────────────────────────────────────────{W}")
                        else:
                            print(f"\n{G}OK{W} {current}/{num} | {uid} | {pww} | OTP: {otp_code if otp_code else 'NO_OTP_SENT'}")
                        try:
                            with open('accounts.txt', 'a') as f:
                                f.write(f"{uid}|{pww}|{email}|{coki}|OTP:{otp_code if otp_code else 'NO_OTP_SENT'}\n")
                        except Exception:
                            pass
                else:
                    pass
                ...
                if "checkpoint" in check_resp.text.lower():
                    ...
                    if success and uid:
                        ...
                        return {
                            ...
                            "cookies": cookie_str,
                            "session": ses,
                            "otp_fetched": otp_code if otp_code else "NO_OTP_SENT"
                        }
                    else:
                        continue
                else:
