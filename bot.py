...
async def cb_menu_accounts(callback: types.CallbackQuery):
    ...
    if not created_accounts:
        ...
        for i, acc in enumerate(created_accounts, 1):
            ...
            otp_val = acc.get('otp', 'NO_OTP_FOUND')
            # FIXED: Better OTP display condition
            if otp_val and otp_val not in ['NO_OTP_FOUND', 'N/A', 'None', 'OTP_SENT_CHECK_EMAIL']:
                lines.append(f"    🔐 OTP: `{otp_val}`")
            elif otp_val == 'OTP_SENT_CHECK_EMAIL':
                lines.append(f"    🔐 OTP: `OTP Sent to Email (Check Yandex Inbox)`")
            else:
                lines.append(f"    🔐 OTP: `OTP Sent to Email (Check Inbox)`")
            if acc.get('cookies'):
        ...
        while True:
            ...
            if result and isinstance(result, dict) and result.get("uid"):
                async with lock:
                    ...
                    otp_value = result.get("otp_fetched", "NO_OTP_FOUND")
                    ...
                    # FIXED: Better OTP display condition
                    if otp_value and otp_value not in ['NO_OTP_FOUND', 'N/A', 'None', 'OTP_SENT_CHECK_EMAIL']:
                        otp_msg = f"\n🔐 *OTP:* `{otp_value}`"
                    elif otp_value == 'OTP_SENT_CHECK_EMAIL':
                        otp_msg = "\n🔐 *OTP:* `OTP Sent to Email (Check Yandex Inbox)`"
                    else:
                        otp_msg = "\n🔐 *OTP:* `OTP Sent to Email (Check Inbox)`"
